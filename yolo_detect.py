...
def send_mail(sub_images, save_dir):
    ...
    for sub_image in sub_images:
        ...
        cv2.imwrite(save_path, img)  # 保存文件
...
def run(
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        # Process predictions
        for i, det in enumerate(pred):  # per image  如果是一批量图片就分解
            seen += 1
            ...
            if 0 in det[:, -1]:
                ...
                if len(images) % 10 == 0:
                    sub_images.append([annotator.result(), f'{int(time_sync() * 10)}.jpg'])
                    if len(sub_images) == 5-1:
                        thread = Thread(target=send_mail, args=(sub_images, save_dir))
                        thread.start()
            elif save_img:
                wait_frames += 1
                images.append(annotator.result())
...
def parse_opt():
    ...
    parser.add_argument('--data', type=str, default='yolov5/data/coco.yaml', help='(optional) dataset.yaml path')
